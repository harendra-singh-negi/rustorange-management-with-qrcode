...
<style>
    ...
    width: 39%;
    height: 100%;
    background-image: url("{{ $userBe->special_section_bg ? Uploader::getImageUrl(Constant::WEBSITE_IMAGE, $userBe->special_section_bg, $userBs) : asset('assets/front/img/good-food-shape.png')}}");
...
@section('content')
            ...
            <div class="fress-shape">
                @if ($userBs->home_version == 'slider')
                    @if (!empty($userBe->slider_shape_img))
                     ...
                     @else
                        <img class="lazy"
                            data-src="{{ asset('assets/restaurant/images/shape.png') }}"
                            alt="shape">
                    @endif
                @else
                ...
                <div class="row align-items-center justify-content-center">
                                ...
                                @foreach ($special_product as $sproduct)
                                    <div class="good-food-item white-bg text-center">
                                        <div class="food-shape">
                                          <img class="food-shape-img" src="{{ asset('assets/tenant/img/special_shape.png') }}" >
                                        </div>
                                        ...
                                        @if (!empty(convertUtf8($sproduct->previous_price)))
                                            <del class="preprice">{{ $userBe->base_currency_symbol_position == 'left' ? $userBe->base_currency_symbol : '' }}{{ convertUtf8($sproduct->previous_price) }}{{ $userBe->base_currency_symbol_position == 'right' ? $userBe->base_currency_symbol : '' }}</del>
                                        @endif
                    ...
                    <div class="col-lg-4">
                        <div class="menu-list-content text-right d-none d-lg-block">
                            ...
                            <a href="{{ route('user.front.product', getParam()) }}"><span>{{ convertUtf8($userBe->special_section_subtitle) }}</span></a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    @if ($userBs->testimonial_section == 1)
        <section class="client-area bg_cover pt-105 pb-95 lazy"
            data-bg="{{ Uploader::getImageUrl(Constant::WEBSITE_TESTIMONIAL_IMAGES, $userBe->testimonial_bg_img, $userBs) }}">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-lg-8">
